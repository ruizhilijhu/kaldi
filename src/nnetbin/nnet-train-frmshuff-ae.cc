...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
      ...
      " Compared to nnet-train-frmshuff, it has one more object function for dnn adaptation based on AE,\n"
      "You need to use two extra options to do AE-based adaptation -objective-function=adapt_mse --ae-adaptation-model"
      "Usage:  nnet-train-frmshuff-ae [options] <feature-rspecifier> <targets-rspecifier> <model-in> [<model-out>]\n"
      "e.g.: nnet-train-frmshuff-ae scp:feats.scp ark:posterior.ark nnet.init nnet.iter1\n"
      "       nnet-train-frmshuff-ae --objective-function=adapt_mse --ae-adaptation-model=ae.mdl scp:feats.scp ark:posterior.ark nnet.init nnet.iter1\n";
    ...
    po.Register("objective-function", &objective_function,
        "Objective function : xent|mse|multitask|adapt_mse");
    ...
    po.Register("ae-adaptation-model", &ae_filename,
        "nonupdatable trained-Autoencoder, used to do adaptation.");
    ...
    while (!feature_reader.Done()) {
	  ...
	  // RL : 
	} else if (objective_function == "adapt_mse") {
	  if (ae_filename == "") {
	    KALDI_ERR << "Missing Autoencoder filename for adaptation.";
	  }
	  
	  // 1. get y and y_ae
	  CuMatrix<BaseFloat> ae_out;
	  Nnet ae;
	  ae.Read(ae_filename);
	  ae.Propagate(nnet_out, &ae_out);
	  CuMatrix<BaseFloat> diff_ae_out_in(ae_out);

	  // gradients re-scaled by weights in Eval,
          adapt_mse.Eval(frm_weights, ae_out, nnet_out, &diff_ae_out_in);
	  	  
	  // 2. get (y_ae-y)*(dy_ae/dy)
	  ae.Backpropagate(diff_ae_out_in, &obj_diff);
	  
	  // 3. get obj_diff
	  obj_diff.AddMat(-1.0, diff_ae_out_in);
	  // end RL
	  ...
	  Output ko("fe", false);
	  obj_diff.Write(ko.Stream(), false);
