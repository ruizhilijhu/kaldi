...
int main(int argc, char *argv[]) {
  ...
  try {
	...
	cnt++;
        ...
        if (objective_function == "adapt_mse") {
	  ...
	  // gradients re-scaled by weights in Eval,
          KALDI_VLOG(1) << "current minibatch idx" <<cnt ;
	  ...
	  if (cnt == 2) {
	    Output k1("dbg_nnet_in", false);
	    nnet_in.Write(k1.Stream(), false);
	    
	    
	    Output k2("dbg_nnet_out", false);
	    nnet_out.Write(k2.Stream(), false);
	  
	  
	    Output k3("dbg_frm_weights", false);
	    frm_weights.Write(k3.Stream(), false);
	    
	    Output k4("dbg_diff_ae_out_in", false);
	    diff_ae_out_in.Write(k4.Stream(), false);


	    Output k5("dbg_obj_diff_pre", false);
	    obj_diff.Write(k5.Stream(), false);
	    
	    Output k6("dbg_ae_out", false);
	    ae_out.Write(k6.Stream(), false);
	  ...
	  if (cnt == 2) {
	    // end RL
	    Output k7("dbg_obj_diff", false);
	    obj_diff.Write(k7.Stream(), false);
	  }
