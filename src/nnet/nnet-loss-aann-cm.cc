...
#include "nnet/nnet-loss-aann-cm.h"
...
/* AANN-CM */ 
void AannCm::Eval(const VectorBase<BaseFloat> &frame_weights,
  ...
  KALDI_ASSERT(aann_mlp.OutputDim() == target.NumCols());
  ...
  // compute the different matrix (aann_out_cm_ - target) and frobenius norm
  aann_diff_cm_ = aann_out_cm_;
  if ( minibatches_ == minibatch_idx ) {
    WriteKaldiObject(aann_diff_cm_, "dbg_aann_diff_cm_1", false);
  }  
  aann_diff_cm_.AddMat(-1.0, target);
  if ( minibatches_ == minibatch_idx ) {
    WriteKaldiObject(aann_diff_cm_, "dbg_aann_diff_cm_2", false);
  }  
  
  double frobenius_norm_loss = pow(aann_diff_cm_.FrobeniusNorm(),2);
  ...
  *diff = net_out; // initialize
  diff->AddMatMat(4.0/num_frames, diff_, kNoTrans, aann_diff_cm_, kNoTrans, 0.0);
  if ( minibatches_ == minibatch_idx ) {
