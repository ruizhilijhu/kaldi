...
# This script does aann ncca aann-ncca or aann-cm adaptation 
...
# aann
[ -z $aannDir -a $objective_function != 'ncca' ] && echo "Missing AANN dir!" && exit 1

# dnn
if [ ! -z $mlp ]; then
    numComponents_dnn=`nnet-info ${mlp} | grep num-components | awk '{print $2}'` || exit 1
else
    numComponents_dnn=`nnet-info ${dnnDir}/final.nnet | grep num-components | awk '{print $2}'` || exit 1
fi
...
if [ $stage -le 0 ]; then
    ...
    # copy dnn dir
    [ ! -d $outdir ] && rsync -avr --exclude '*decode*' $dnnDir/ $outdir 
	...
	[ -f $outdir/final.nnet ] && rm $outdir/final.nnet
    	nnet-set-learnrate --components=`seq -s ":" $numLayersFreezeStart $numLayersFreeze` --coef=0 ${outdir}/final.nnet.tmp $outdir/final.nnet 
    fi
    ...
    if [ $objective_function != 'ncca' ];then # AANN: copy aann.nnet and aann.ft and make aann.nnet non-updatable
...
echo ============================================================================
outdir=${expid}/preAdapt-dnn/bn_decode # the only output
if [ $stage -le 1 ]; then
    mkdir -p $outdir
...
echo ============================================================================
echo "      Pre-Adaptation: Compute MSE, FRM, FN for clean and adapted data"
echo ============================================================================
outdir=${expid}/preAdapt-dnn # the only output
if [ $stage -le 2 ]; then
    for d in ${decode_subsets}; do
	for n in ${decode_noises}; do
	    ...
	    if [ $objective_function != 'ncca' ];then
		...
		echo "Pre-Adaptation Decoding: aannFN"
		out_fn=$outdir/aannfn_decode/$d/$n		
		mkdir -p $out_fn			    
		...
		local/ncca/compute_fn_loss.sh \
		    --minibatch_size $minibatch_size \
		    --cmd "$decode_cmd" \
		    --mlp $outdir/final.nnet.1 \
		    ${data_fbank_allnoise}/$d/$n ${ref_cm} $outdir $out_fn || exit 1       
	    fi
	done
    done
fi
...
echo ============================================================================
echo "                 Adaptation based on $noise ----${objective_function}     "
echo ============================================================================
...
if [ $stage -le 3 ]; then
    ...
    train_options="${train_options} --max-iters $max_iters --l1-penalty ${l1_penalty} --l2-penalty ${l2_penalty}"
	...
	train_tool="nnet-train-frmshuff-ncca --objective-function=${objective_function} --ncca-ref-mat=${ref_cm}"
    fi    
    ...
    $cuda_cmd $outdir/log/train_ncca.log \
	      ...
	      ${dropout_schedule:+ --scheduler-opts "--dropout-schedule $dropout_schedule"} \
...
if [ $stage -le 4 ]; then
	...
	if [ ${objective_function} != 'ncca' ];then
	    ...
	    ln -s $PWD/${outdir}/nnet/${nnet_id} $out/final.nnet.1 # final.nnet.1
	fi
