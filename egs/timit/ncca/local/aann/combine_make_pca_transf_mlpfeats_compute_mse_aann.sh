...
[ -z $mlp ] && mlp=$transf_dir/feature_extractor.nnet
# Check required 
required="$srcdata/feats.scp"
required=$required" $mlp $transf_dir/final.feature_transform $transf_dir/pca.mat"
...
# cmvn_opts
[[ -z $aann_cmvn_opts ]]&& aann_cmvn_opts='--norm-means=false --norm-vars=false'

if [[ $utt2utt == "utt2utt" ]]; then
    for JOB in `seq 1 $nj`;do
	paste <(awk '{print $1}' ${sdata}/$JOB/utt2spk) <(awk '{print $1}' ${sdata}/$JOB/utt2spk) > ${sdata}/$JOB/spk2utt_utt2utt     || exit 1
    done
fi
...
$cmd JOB=1:$nj $logdir/make_pca_transf.JOB.log \
  ...
  compute-cmvn-stats --spk2utt=ark:${sdata}/JOB/spk2utt${utt2utt:+'_utt2utt'} scp:\$tmp_dir/pca_transf_$name.JOB.scp \
  ark,scp:\$tmp_dir/pca_transf_$name.JOB.cmvn.ark,\$tmp_dir/pca_transf_$name.JOB.cmvn.scp '&&' \
  nnet-score-sent --use-gpu=no --objective-function=mse --feature-transform=${aann_dir}/final.feature_transform \
  ${aann_dir}/final.nnet \
  "ark,s,cs:copy-feats scp:\$tmp_dir/pca_transf_$name.JOB.scp ark:- | apply-cmvn ${aann_cmvn_opts} --utt2spk=ark:${sdata}/JOB/utt2spk${utt2utt:+'_utt2utt'} scp:\$tmp_dir/pca_transf_$name.JOB.cmvn.scp ark:- ark:- |" \
  "ark,s,cs:copy-feats scp:\$tmp_dir/pca_transf_$name.JOB.scp ark:- | apply-cmvn ${aann_cmvn_opts} --utt2spk=ark:${sdata}/JOB/utt2spk${utt2utt:+'_utt2utt'} scp:\$tmp_dir/pca_transf_$name.JOB.cmvn.scp ark:- ark:- | nnet-forward ${aann_dir}/final.feature_transform ark:- ark:- | feat-to-post ark:- ark:- |" \
  ark:- \| select-feats 0 ark:- ark,scp:${mse_data_dir}/mse.JOB.ark,${mse_data_dir}/mse.JOB.scp || exit 1; 
